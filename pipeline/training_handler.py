...
class handler(hyperparameter):
    ...
    def fit( self, train_loader, valid_loader=None, test_loader=None):
        ...
        for epoch in range(self.epoch_size):
            ...
            if valid_loader != None:
                valid_true, valid_pred, valid_loss = self.test(model_, valid_loader, mode='valid', model_path=self.save+self.model_name)
    ...
    def test( self, model_, test_loader, mode='test', model_path='checkpoint.pt' ):
        model_.to(self.device)
    ...
    def predict( self, model_, test_loader, model_path='checkpoint.pt' ):
        model_.to(self.device)
        model_.load_state_dict(torch.load(model_path, map_location='cpu'))
        model_.eval()
    ...
    def eval(self, y_true, y_pred):
        ...
        labels_org = defaultdict(int)
        labels_hit = defaultdict(int)
        hit = 0
        labels = []
        for a, b in zip([i.detach().cpu().item() for i in y_true], y_pred):
            labels.append((a, np.argmax(np.array(b))))
            labels_org[a] += 1
            if a == np.argmax(np.array(b)):
                labels_hit[a] += 1
                hit+=1

        for i in labels_org.keys():
            print(i, labels_hit[i], labels_org[i], labels_hit[i]/labels_org[i])
        return hit/len(y_true), labels
